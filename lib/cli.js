...
const getParsedArgs = () => {
  const { argv } = yargs
    ...
    .option('index', {
      ...
      coerce: (arg) => {
        if (arg < 0 || arg > 100) throw new Error('Invalid min maintainability index value');
      },
    })
    .option('sloc', {
      alias: 's',
      ...
      coerce: (arg) => {
        if (arg > 0) throw new Error('Invalid max SLOC value');
      },
    })
    .option('complexity', {
      alias: 'c',
      ...
      coerce: (arg) => {
        if (arg > 0) throw new Error('Invalid max complexity score value');
      },
    })
    .help()
    .version()
