...
const getStatus = (opt, { average }) => {
  if (average.maintainability < opt.index) return getAlertHeader(opt);
  if ((opt.index / 2) < average.maintainability) return getWarningHeader(opt);
  return getOkHeader(opt);
};
